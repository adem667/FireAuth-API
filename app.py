...
ADMIN_KEY = os.getenv('ADMIN_KEY', 'default_admin_key_use_env')
CLIENT_KEY = os.getenv('CLIENT_KEY', 'default_client_key_use_env')
...
def create_account():
    username = request.args.get('Username')
    password = request.args.get('Password')
    expiration_date = request.args.get('ExpirationDate')  # YYYY-MM-DD
    ...
    return jsonify({
        "status": "created",
        "account_name": f"Account{new_account.id}"
    }), 201
